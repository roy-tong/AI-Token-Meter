...
| REQ-20260906-002 | 紧凑浮动条与渐进式交互 | 按 CodeNotch 研究中的推荐方向改进 AI Token Meter：双平台增加 Compact/Comfortable 密度并默认使用 Compact、保留深海背景和 Logo-only；增加可选闲置折叠、粗粒度 Working/Waiting/Idle 双轨状态、Provider 显示与排序、统一数据可信度/新鲜度、持久化刷新退避和浮动条右键快捷菜单 | 高 | 进行中 | 2026-09-06 | 先完成设计规格与分阶段测试驱动计划；不得破坏现有左右贴边、多屏位置、全屏隐藏、详情自动隐藏、Provider 品牌色、隐私边界和 macOS/Windows 同步规则 | [竞品研究](development/2026-09-06-codenotch-competitive-review.md) · 待补设计规格、实施计划、开发日志和 Git 证据 |
