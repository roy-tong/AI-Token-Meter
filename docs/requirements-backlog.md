...
| REQ-20260906-003 | CI可靠性 | 追踪GitHub runner上偶发的macOS PTY父进程退出超时/并发输出缺失，以及Windows ConPTY采集超时；不删除断言或跳过测试 | 低 | 受环境限制 | 2026-09-06 | 当前最终双平台CI全绿、本机连续复跑无法重现；保留原失败日志，后续同类失败先增加脱敏进程时序诊断再定位，不能宣称根因已修复 | [开发记录](development/2026-09-06-compact-progressive-strip.md) · 失败CI `34033000263`、`34033000265` |
